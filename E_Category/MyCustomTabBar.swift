...
class MyCustomTabBarController: UITabBarController {
    ...
    func addSomeTabItems() {
        let vc1 = UINavigationController(rootViewController: AdditivesViewController())
...
extension UIColor {
    public convenience init?(hex: String , alpha: Double = 1.0) {
        ...
        var hexNumber: Int64 = 0
        
        if scanner.scanHexInt64(&hexNumber) {
            self.init(red: CGFloat((hexNumber & 0xFF0000) >> 16) / 255.0, green: CGFloat((hexNumber & 0x00FF00) >> 8) / 255.0, blue: CGFloat(hexNumber & 0x0000FF) / 255.0, alpha: CGFloat(1.0))
            return
        }
        return nil
    }
